...
# EXTENT / STRINGS / LANG / applyLang are all in scope.
#
# Strings this block must never contain, because site/viewer_upgrade.py
# patches the last <script> by literal str.replace against them:
#   "  function animate() {", "    renderer.render(scene, camera);",
#   "  const camera = new THREE.PerspectiveCamera(",
#   "  const renderer = new THREE.WebGLRenderer(",
#   '  window.addEventListener("resize", resize);',
#   "  const HOVER_NODES = [];", "    camera.updateProjectionMatrix();",
#   "window.innerWidth / window.innerHeight"
# and the ids naRail / naViewport / neuroNav, which make the site layer
# bail out. tests/test_connectivity.py asserts all of that.
# --------------------------------------------------------------------------
CUSTOM_JS = r"""
  (function connectivityLayer() {
    // __CONN_JSON__ carries {graph, systems}. The graph stays the pure
    // connectivity schema written to connectivity.json; systems is the
    // display-name table (SYSTEM_NAMES) only the picker grouping needs.
    ...
    style.textContent = [
      ...
      "#mouseStage #mouseStart[aria-pressed='true']{border-color:#84dcc5;background:rgba(132,220,197,.14)}",
    ...
    stage.innerHTML = [
      ...
      '<div class="ms-btns">',
      '<button type="button" id="mouseStart" aria-pressed="false" data-conn-en="Run wheel" data-conn-zh="跑動滾輪"></button>',
